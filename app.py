...
from classes import Table, Column, Row, get_tables_product
from logger import logger
from ops import load_db_from_request, save_db_to_file
...
def product():
    db = load_db_from_request(request)
    return render_template('product.html', db_name=db.name, tables=db.list_tables())
...
def get_product():
    ...
    response = {'atable1': table1.to_json(), 'atable2': table2.to_json()}

    if set([i.name for i in table1.columns]) != set([i.name for i in table2.columns]):
        res = None
    else:
        res = get_tables_product(table1, table2)

    response['res'] = res.to_json() if res else None
...
def get_table(table_name):
    ...
    return render_template('table.html', table_name=table.name, rows=table.rows, columns=table.columns)
